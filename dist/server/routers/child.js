...
const child_1 = require("../schemas/child");
...
router.get('/', async (req, res) => {
    const allChildren = await db_1.AppDataSource.getRepository(Child_1.Child).find();
...
router.post('/', async (req, res) => {
    try {
        const validatedData = child_1.CreateChildSchema.parse(req.body);
        const parents = await db_1.AppDataSource.getRepository(Parent_1.Parent).find({
            where: {
                id: (0, typeorm_1.In)(validatedData.parentIds)
            }
        });
        ...
        const station = await db_1.AppDataSource.getRepository(Station_1.Station).findOne({
            where: {
                ...
                type: types_1.StationType.SCHOOL
            }
        });
        if (!station) {
            return res.status(404).json({ message: "Station does not exist or it isn't labeled as school" });
        }
        const child = await db_1.AppDataSource.getRepository(Child_1.Child).insert(validatedData);
        ...
        await db_1.AppDataSource.getRepository(ParentChild_1.ParentChild).insert(parentChildConnector);
...
router.put('/:id', async (req, res) => {
    try {
        const childId = req.params.id;
        const validatedData = child_1.UpdateChildSchema.parse(req.body);
        const child = await db_1.AppDataSource.getRepository(Child_1.Child).findOne({
            where: { id: childId }
        });
        if (!child) {
        ...
        await db_1.AppDataSource.getRepository(Child_1.Child).update(child.id, {
            ...validatedData,
            updatedAt: new Date()
        });
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        await db_1.AppDataSource.getRepository(ParentChild_1.ParentChild).delete({ childId: childId });
        await db_1.AppDataSource.getRepository(Child_1.Child).delete(child.id);
