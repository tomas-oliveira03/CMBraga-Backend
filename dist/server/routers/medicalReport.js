...
const medicalReport_1 = require("../schemas/medicalReport");
...
router.get('/', async (req, res) => {
    const allReports = await db_1.AppDataSource.getRepository(MedicalReport_1.MedicalReport).find();
...
router.get('/:id', async (req, res) => {
    ...
    const report = await db_1.AppDataSource.getRepository(MedicalReport_1.MedicalReport).findOne({
        where: {
            id: reportId
        }
    });
...
router.post('/', async (req, res) => {
    try {
        const validatedData = medicalReport_1.CreateMedicalReportSchema.parse(req.body);
        const child = await db_1.AppDataSource.getRepository(Child_1.Child).findOne({
        ...
        await db_1.AppDataSource.getRepository(MedicalReport_1.MedicalReport).insert(validatedData);
...
router.put('/:id', async (req, res) => {
    try {
        const reportId = req.params.id;
        const validatedData = medicalReport_1.UpdateMedicalReportSchema.parse(req.body);
        const report = await db_1.AppDataSource.getRepository(MedicalReport_1.MedicalReport).findOne({
            where: { id: reportId }
        });
        if (!report) {
        ...
        await db_1.AppDataSource.getRepository(MedicalReport_1.MedicalReport).update(report.id, {
            ...validatedData,
            updatedAt: new Date()
        });
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        await db_1.AppDataSource.getRepository(MedicalReport_1.MedicalReport).delete(report.id);
