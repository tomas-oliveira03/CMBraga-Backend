...
const issue_1 = require("../schemas/issue");
...
router.get('/', async (req, res) => {
    const allIssues = await db_1.AppDataSource.getRepository(Issue_1.Issue).find();
...
router.get('/:id', async (req, res) => {
    ...
    const issue = await db_1.AppDataSource.getRepository(Issue_1.Issue).findOne({
        where: { id: issueId }
    });
    if (!issue) {
...
router.post('/', async (req, res) => {
    try {
        const validatedData = issue_1.CreateIssueSchema.parse(req.body);
        ...
        await db_1.AppDataSource.getRepository(Issue_1.Issue).insert(validatedData);
...
router.put('/:id', async (req, res) => {
    try {
        const issueId = req.params.id;
        const validatedData = issue_1.UpdateIssueSchema.parse(req.body);
        const issue = await db_1.AppDataSource.getRepository(Issue_1.Issue).findOne({
            where: { id: issueId }
        });
        if (!issue) {
        ...
        await db_1.AppDataSource.getRepository(Issue_1.Issue).update(issue.id, {
            ...validatedData,
            updatedAt: new Date()
        });
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        await db_1.AppDataSource.getRepository(Issue_1.Issue).delete(issue.id);
