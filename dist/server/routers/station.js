...
const station_1 = require("../schemas/station");
...
router.get('/', async (req, res) => {
    const allStations = await db_1.AppDataSource.getRepository(Station_1.Station).find();
...
router.post('/', async (req, res) => {
    try {
        const validatedData = station_1.CreateStationSchema.parse(req.body);
        await db_1.AppDataSource.getRepository(Station_1.Station).insert(validatedData);
...
router.put('/:id', async (req, res) => {
    try {
        const stationId = req.params.id;
        const validatedData = station_1.UpdateStationSchema.parse(req.body);
        const station = await db_1.AppDataSource.getRepository(Station_1.Station).findOne({
            where: { id: stationId }
        });
        if (!station) {
        ...
        await db_1.AppDataSource.getRepository(Station_1.Station).update(station.id, {
            ...validatedData,
            updatedAt: new Date()
        });
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        await db_1.AppDataSource.getRepository(Station_1.Station).delete(station.id);
